...
"""Generate the HalfNutELS brand mark in lib/display/icons/ as an LVGL v9 A8 image.
    ...
    halfNutLogo    96x96    the split nut
...
other, and a plain bore through the middle.

Two things have to survive at 96 px on a panel read from a lathe's operating
position, and both are carried by silhouette rather than by detail:
...
The bore is SMOOTH, and deliberately so. An earlier revision put thread crests
in it, on the reasoning that the thread is what says "leadscrew" -- but this is
a half-nut pair viewed from the side, and from the side you do not see the
thread. Drawing it was drawing something that is not there, and it cost twice
over: the crests also read as gear teeth at this size, which is the wrong signal
entirely for a device whose whole purpose is deleting a lathe's change gears.
The nut and the split carry the idea on their own.
...
RASTERISATION
-------------
Union of the adds minus union of the subs, sampled at 8x8 subpixels and box
downsampled, giving real antialiased alpha. The nut is rasterised WHOLE and only then split,
...
def nut_shapes():
    """(additive, subtractive) shape lists for the WHOLE, unsplit nut."""
    return [hexagon()], [Circle(CX, CY, BORE_R)]
...
def rasterise(adds, subs):
    """Return a bytearray of W*H alpha values for (adds - subs)."""
    ...
    for sy in range(H * SS):
        y = sy * step + half
        spans = subtract(merge([s for sh in adds for s in sh.spans(y)]),
...
def main():
    adds, subs = nut_shapes()
    data = split_halves(rasterise(adds, subs))
