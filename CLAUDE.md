...
- The mark is generated by **`scripts/make_logo.py`** (a sibling of `make_glyphs.py`, importing its
  ...
  re-run it; never hand-edit the `.c`. Two gotchas it now documents: the bore must be subtracted from
  the body BEFORE the thread crests are unioned on (otherwise the subtraction eats them), and the crest
  count is deliberately low — enough to read as thread, few enough not to read as a **gear**, which
  would be the wrong signal for a device that deletes a lathe's change gears.
